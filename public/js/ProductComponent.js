        ...
        this.$parent.getJson(`/api/products`)
            .then(data => {
                for (let item of data){
                    item.img = `img/product_img/${item.id}.jpg`;
...
Vue.component('product', {
    ...
    template: `
            <div class="products__card">
                        <a href="product.html"><img class="card__image" :src="img" alt="product image"></a>
                        <a href="product.html">
                            <h3 class="card__h3">{{product.name}}</h3>
                        </a>
                        <p class="card__text">{{product.description}}</p>
                        <p class="card__price">$ {{product.price}}</p>
                        <button @click="$emit('add-product', product)" class="product__add"><svg class="product__cart" width="27" height="25"
